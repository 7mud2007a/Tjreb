...
## If the page appears as only text

The 3D scene is loaded as ES modules, so it must be served by an HTTP server. On Render Static Site, the files can be served directly from the repository root.
...
The latest prototype uses ESM-hosted Three.js/GSAP imports so Three.js addon dependencies resolve correctly in a browser.
